...
namespace Modules\Media\Admin {
    class Dummy implements \Framework\Install\DummyInterface {
        public static function generate($db, $amount) {
            ...
            while($amount > 1) {
                $dataString .= " ( '" . \Framework\Utils\RnG\String::generateString(5, 15) . "', '', '" . $fileTypes[rand(0, count($fileTypes) - 1)] . "', 1, '" . \Framework\Utils\RnG\DateTime::generateRandomDate('2005-12-10', '2014-12-31')->format('Y-m-d H:i:s') . "' ),";
                $amount--;
            }
